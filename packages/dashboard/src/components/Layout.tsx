    ...
    <div className="flex h-screen overflow-hidden">
      <nav className="w-56 flex-shrink-0 flex flex-col border-r"
        ...
        <div className="p-5 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <h1 className="text-lg font-semibold tracking-tight" style={{ color: 'var(--color-primary-light)' }}>
            AIL
          </h1>
        ...
        <div className="flex-1 py-3">
          {navItems.map(item => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-5 py-2.5 text-sm transition-colors ${isActive ? 'font-medium' : ''}`
              }
              style={({ isActive }) => ({
